...
public class LastSlotScript : MonoBehaviour
{
    ...
    {
        if (slotManager.GetComponent<LastSlotManagerScript>().m_stopSlot)
        {
            m_HitBullet = true;
        }

        if (!slotManager.GetComponent<LastSlotManagerScript>().m_stopFirstSlot)
        {
            if (!m_HitBullet)
            {
        ...
        else
        {
            ...
            else if (slotNum != slotManager.GetComponent<LastSlotManagerScript>().m_slotNum)
            {
                if (doSlot)
                {
                    StartCoroutine(SlotTime());
